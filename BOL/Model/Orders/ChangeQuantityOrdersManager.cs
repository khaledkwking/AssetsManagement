...
namespace BOL
{
    public class ChangeQuantityOrdersManager : Repository<ChangeQuantityOrders>
    {

        public ChangeQuantityOrdersManager(GPFAssetsEntities ctx) : base(ctx)
        {

        }
        public override bool Delete(ChangeQuantityOrders entity)
        {
        ...
        {
            ChangeQuantityOrders st = GetById(id);

            return Update(st);
        }
        public List<ChangeQuantityOrders> GetCastByName(string UnitSearch)
        {
        ...
        public List<ChangeQuantityOrders> GetNotDelAllByUserId(int userId)
        {
            UnitOfWork UWork = new UnitOfWork();
            List<Room_tbl> InventoriesList = UWork.RoomsManager.GetInventoriesAll().ToList();
            ...
            List = GetAll().Where(c => c.IsDeleted == false || c.IsDeleted == null).ToList();
            List<ChangeQuantityOrders> NewList = List.Where(c => listOfRoomId.Contains(c.StoreId_From.GetValueOrDefault())).ToList();
