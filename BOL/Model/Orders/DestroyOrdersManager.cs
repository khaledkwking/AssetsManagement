...
namespace BOL
{
    public class DestroyOrdersManager : Repository<DestroyOrders>
    {

        public DestroyOrdersManager(GPFAssetsEntities ctx) : base(ctx)
        {

        }
        public override bool Delete(DestroyOrders entity)
        {
        ...
        {
            DestroyOrders st = GetById(id);

            return Update(st);
        }
        public List<DestroyOrders> GetCastByName(string UnitSearch)
        {
        ...
        public List<DestroyOrders> GetNotDelAllByUserId(int userId)
        {
            UnitOfWork UWork = new UnitOfWork();
            List<Room_tbl> InventoriesList = UWork.RoomsManager.GetInventoriesAll().ToList();
            ...
            List<DestroyOrders> NewList = List.Where(c => listOfRoomId.Contains(c.StoreId_From.GetValueOrDefault())).ToList();
