...
{
    public class ReportsTransferOrdersController : Controller
    {
        string CurrentUser { get; set; }
        public static string QRCode;
        private UnitOfWork unitWork = new UnitOfWork();
        
        public ActionResult TransferOrdersItemReport()
        {
            ...
            model.InventoriesTo = new SelectList(allStoresList, "Room_Id", "Room_Name", model.StoreToId);
        ...
        [HttpPost]
        public ActionResult TransferOrdersItemReport(ReportViewModel model)
        {

         int? SupplierId=model.supplierId ;
         long? StoreId=model.StoreId ;
            long? StoreToId = model.StoreToId;
         ...
         //DateTime ? ToDate= model.ToDate == null ? DateTime.Today : DateTime.Parse(model.ToDate);
            ...
            int userId = SesssionUser.GetCurrentUserId();
            List<vwTransferOrdersDetails> cm = unitWork.vwTransferOrdersDetailsManager.GetNotDelAllByParam
         (FromDate, ToDate,StoreId, StoreToId,ItemId, CatMain_Id, userId).ToList();// (ItemId, DeptId, RoomId, EmpId).ToList();
            //string ReportName,string DataSetName, IEnumerable dataSourceValue
            TempData["list"] = cm;
            string ReportName = "TransferOrderItemsRpt.rdlc";
           switch (searchType)
            {
                case 1:
                    ReportName = "TransferOrderItemsRpt.rdlc";
                    break;
                case 2:
                    ReportName = "TransferOrderTotalItemsRpt.rdlc";
                    break;
            }
        ...
        public ActionResult ShowTransferOrderReport(long? orderId)
        {
         
           List<vwTransferOrdersDetails> cm = unitWork.vwTransferOrdersDetailsManager.GetNotDelByOrderId(orderId);
           if (cm.Count >0)
            {
                ...
                CurrentUser + "\n" + "OrderId: " + 
                cm[0].TransferOrderId.ToString() + "\n" + "Order Date :"+
                 cm[0].TransferOrderDate.Value.ToShortDateString();
